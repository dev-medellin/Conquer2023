function setModalMaxHeight(e) {
    ...
    var t = this.$content.outerHeight() - this.$content.innerHeight(), n = $(window).width() < 768 ? 20 : 60, a = $(window).height() - (n + t), o = this.$element.find(".modal-header").outerHeight() || 0, i = this.$element.find(".modal-footer").outerHeight() || 0, s = a - (o + i); this.$content.css({ overflow: "hidden" }), this.$element.find(".modal-body").css({ "max-height": s, "overflow-y": "auto" })
